...
def check_input_file(in_file, description, extensions):
    ...
    if not os.path.isfile(in_file):
        ...
        if extensions and extension not in extensions:
            errors.append(
                "Wrong file extension for " + description + " file provided: " + in_file + ". Acceptable extensions are: " + ",".join(
                    extensions))
    return errors
...
def driver(gtc_files, manifest_reader, genome_reader, output_vcf_files, expand_identifiers, unsquash_duplicates,
    ...
    locus_entries = LocusEntryFactory(vcf_record_factory, genome_reader.get_contig_order(), unsquash_duplicates,
                                      logger).create_locus_entries(manifest_reader)
    ...
    for (gtc_file, output_vcf_file) in zip(gtc_files, output_vcf_files):
        if gtc_file:
            ...
            if os.path.splitext(os.path.basename(gtc.get_snp_manifest()))[0].lower() != \
                    os.path.splitext(os.path.basename(manifest_reader.source_file))[0].lower():
                logger.warn(
                    "Provided manifest name: " + manifest_reader.source_file + " and manifest file used to generate GTC file: " + gtc.get_snp_manifest() + " do not match, skipping")
                continue
...
def generate_io_files(gtc_paths, output_vcf_path, manifest_file):
    ...
    if os.path.isdir(output_vcf_path):
        if gtc_paths[0] is None:
            output_vcf_files = [
                os.path.join(output_vcf_path, os.path.splitext(os.path.basename(manifest_file))[0] + ".vcf")]
        else:
            output_vcf_files = []
            for gtc_file in gtc_paths:
                # noinspection PyTypeChecker
                output_vcf_files.append(
                    os.path.join(output_vcf_path, os.path.splitext(os.path.basename(gtc_file))[0] + ".vcf"))
    else:
        output_vcf_files = []
        if len(gtc_paths) > 1:
...
def get_manifest_reader(manifest_file, genome_reader, loci_to_filter, skip_indels, logger):
    ...
    if manifest_file.lower().endswith(".bpm"):
        if not skip_indels:
            raise Exception(
                "Must skip indel processing (--skip-indels) when using BPM manifest, use CSV manifest to enable indel processing")
...
def main():
    ...
    parser.add_argument("--gtc-paths", dest="gtc_paths", nargs="+", required=False,
                        help="One or more GTC files to process (optional)")
    parser.add_argument("--manifest-file", dest="manifest_file", required=True,
                        help="Bead pool manifest for product (*.csv or *.bpm)")
    parser.add_argument("--genome-fasta-file", dest="genome_fasta_file", required=True,
                        help="Reference genome in fasta format")
    parser.add_argument("--output-vcf-path", dest="output_vcf_path", default="output.vcf", required=False,
                        help="Path for generation of VCF output (default is output.vcf)")
    parser.add_argument("--skip-indels", dest="skip_indels", action="store_true", default=False,
                        help="Skip processing of indels (default is False)")
    parser.add_argument("--log-file", dest="log_file", default=None, required=False,
                        help="File to write logging information (optional)")
    parser.add_argument("--expand-identifiers", dest="expand_identifiers", action="store_true", default=False,
                        help="For VCF entries with multiple corresponding manifest entries, list all manifest identifiers in VCF ID field")
    parser.add_argument("--unsquash-duplicates", dest="unsquash_duplicates", action="store_true", default=False,
                        help="Generate unique VCF records for duplicate assays")
    parser.add_argument("--auxiliary-loci", dest="auxiliary_loci", default=None, required=False,
                        help="VCF file with auxiliary definitions of loci (optional)")
    parser.add_argument("--filter-loci", dest="filter_loci", default=None, required=False,
                        help="File containing list of loci names to filter from input manifest (optional)")
    parser.add_argument("--disable-genome-cache", dest="disable_genome_cache", default=False, action="store_true",
                        help="Disable caching of genome reference data")
    ...
    try:
        ...
        manifest_reader = get_manifest_reader(args.manifest_file, genome_reader, loci_to_filter, args.skip_indels,
                                              logger)
        auxiliary_records = read_auxiliary_records(args.auxiliary_loci)

        driver(gtc_paths, manifest_reader, genome_reader, output_vcf_files, args.expand_identifiers,
               args.unsquash_duplicates, auxiliary_records, args.include_attributes, logger)
